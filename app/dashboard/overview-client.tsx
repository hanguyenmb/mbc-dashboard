...
export function OverviewClient({ userName, monthlyData, serviceMonthly, revenueType }: OverviewClientProps) {
  ...
  // Tổng kết quả thực tế tháng có data
    ...
    <div>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
          {[
            { label: "Tổng thực hiện (3T)", value: `${tongThucHien.toFixed(2)} tỷ`, sub: "HN + HCM", color: "border-blue-500/20 text-blue-400" },
            { label: "Mục tiêu 8% (3T)",    value: `${tongMt8.toFixed(2)} tỷ`,       sub: `Đạt ${tlMt8.toFixed(1)}%`,  color: tlMt8 >= 100 ? "border-green-500/20 text-green-400" : "border-amber-500/20 text-amber-400" },
            { label: "Mục tiêu 10% (3T)",   value: `${tongMt10.toFixed(2)} tỷ`,      sub: `Đạt ${tlMt10.toFixed(1)}%`, color: tlMt10 >= 100 ? "border-green-500/20 text-green-400" : "border-amber-500/20 text-amber-400" },
        ...
        <Card className="mt-4">
          ...
          <CardContent>
            ...
            {/* HN / HCM breakdown table */}
            <div className="mt-6 overflow-x-auto rounded-xl border border-slate-700/50">
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-800/80">
                    <td className="py-2.5 px-3 font-semibold text-slate-400 border-b border-slate-700/60 min-w-[130px]">Nhóm DV</td>
                    {["T1", "T2", "T3"].map((m) => (
                      <td key={`hn-${m}`} colSpan={1} className="py-2.5 px-2 text-right font-semibold text-blue-400 border-b border-slate-700/60 min-w-[60px]">
                        HN {m}
                      </td>
                    ))}
                    {["T1", "T2", "T3"].map((m) => (
                      <td key={`hcm-${m}`} colSpan={1} className="py-2.5 px-2 text-right font-semibold text-cyan-400 border-b border-slate-700/60 min-w-[60px]">
                        HCM {m}
                      </td>
                    ))}
                    <td className="py-2.5 px-3 text-right font-bold text-white border-b border-slate-700/60 min-w-[70px]">Q1 Tổng</td>
                  </tr>
                </thead>
                <tbody>
                  {SVC_KEYS.map((s, si) => {
                    const q1total = SERVICE_MONTHLY.reduce((sum, m) => sum + ((m as any)[s.key] as number), 0);
                    return (
                      <tr key={s.key} className={si % 2 === 0 ? "bg-slate-800/30 border-b border-slate-700/30" : "bg-slate-900/20 border-b border-slate-700/30"}>
                        <td className="py-2 px-3 font-semibold text-slate-200" style={{ borderLeft: `3px solid ${s.color}` }}>
                          {s.label}
                        </td>
                        {SERVICE_MONTHLY.map((m) => (
                          <td key={`hn-${m.month}`} className="py-2 px-2 text-right tabular-nums text-blue-200">
                            {((m.hn as any)[s.key] as number).toFixed(1)}
                          </td>
                        ))}
                        {SERVICE_MONTHLY.map((m) => (
                          <td key={`hcm-${m.month}`} className="py-2 px-2 text-right tabular-nums text-cyan-200">
                            {((m.hcm as any)[s.key] as number).toFixed(1)}
                          </td>
                        ))}
                        <td className="py-2 px-3 text-right font-bold tabular-nums text-white">
                          {q1total.toFixed(1)}
                        </td>
                      </tr>
                    );
                  ...
                  <tr className="bg-slate-700/40 font-bold">
                    ...
                    {SERVICE_MONTHLY.map((m) => {
                      const hnTotal = SVC_KEYS.reduce((sum, s) => sum + ((m.hn as any)[s.key] as number), 0);
                      return (
                        <td key={`hn-total-${m.month}`} className="py-2.5 px-2 text-right tabular-nums text-blue-300">
                          {hnTotal.toFixed(1)}
                        </td>
                      );
                    })}
                    {SERVICE_MONTHLY.map((m) => {
                      const hcmTotal = SVC_KEYS.reduce((sum, s) => sum + ((m.hcm as any)[s.key] as number), 0);
                      return (
                        <td key={`hcm-total-${m.month}`} className="py-2.5 px-2 text-right tabular-nums text-cyan-300">
                          {hcmTotal.toFixed(1)}
                        </td>
                      );
                    })}
                    <td className="py-2.5 px-3 text-right tabular-nums text-white">
                      {SVC_KEYS.reduce((sum, s) => sum + SERVICE_MONTHLY.reduce((s2, m) => s2 + ((m as any)[s.key] as number), 0), 0).toFixed(1)}
                    </td>
                  </tr>
                </tbody>
              </table>
